...
- **A second approval made while the host was working was silently deleted** by the first run's clean-up, because the request was consumed at the end. The updater now renames it to `update-applying` the moment it starts — the Dashboard shows *the host is applying it now*, the path unit cannot re-fire, and a new request survives. The console also allows one deployment at a time (a second is refused while one is pending or applying), but replaces a request the host has not touched in ten minutes, so a missing updater cannot lock the console out.
- **Rollback could leave the container down** if the registry went away between the pull and the failure: the re-pull of the previous tag failed and the recreate behind it never ran. The previous image is local; the updater now recreates from it regardless.
- **`deploy.sh 1.99.0` rewrote the pin before finding out the version does not exist**, leaving the compose file pointing at nothing for the next CasaOS recreate. It now proves the manifest exists first, and writes the same `update-result` the updater does, so a deploy made from the host clears a stale *rolled back* verdict on the Dashboard — which otherwise stayed there for good.
- **A bad `OMNISSA_UPDATE_CHECK_INTERVAL` stopped the application from starting** — `@Scheduled` parses the property itself, and an approval tool that will not boot over an update-check typo is the wrong trade. An unreadable value now falls back to `P1D` with a warning; anything under `PT5M` is raised to `PT5M` so a typo cannot hammer the registry from every install that made it.
- Two approvals racing past the pending check shared one temp filename; the intent is now written through a uniquely named temp file. A `reason` in `update-result` is forced onto one line and the reader takes the first occurrence of each key, so a host error message can never redefine the verdict.
- The updater and `deploy.sh` share a lock, so a console approval and a hand-run deploy cannot interleave edits to the same compose file. The updater no longer runs `docker image prune` — that pruned every dangling image on a shared host, and the previous tag is wanted in cache for rollback.
- **The update check could throw after all.** A registry error message longer than the status row's column (a captive portal's HTML in a Jackson parse error will do it) failed the save, which sat outside the fail-soft block — the button returned 500, the scheduler failed identically for ever, and the Dashboard kept showing a green, stale *up to date*. The message is truncated and the save is inside the guard; nothing escapes `check()`.
- **An empty tag list wiped every known version.** A registry incident, or a wrong repository, answered with no `N.N.N` tags; the list was overwritten with nothing and no error recorded, so every approval — rollback included — was refused until the registry recovered. The previous list is kept and *no release versions* is recorded as the error.
- **"Check now" raced the scheduler.** Two checks interleaving load→save on the singleton row could lose the once-only announcement stamp and re-announce the same version. One check runs at a time; a second caller gets the last-known result.
- **One notifier throwing discarded the other's delivery.** The e-mail path called a tenant precondition outside its guard; with both channels on, a webhook that delivered was forgotten and the chat channel got the same announcement on every check. Channels are isolated, and the e-mail no longer requires a tenant.
- **The registry answer was read without a cap** and redirects were followed with the token. The body is read through a 1 MB limit, a redirect is refused, `Link: rel="next"` pages are followed (up to ten), and a repository that is not `owner/name` falls back to the default with a warning instead of producing confusing failures.
- **A host-written timestamp outside `Date`'s range** made `GET /api/updates/status` throw for every user; any malformed verdict now reads as *nothing to report*.
- **The Dashboard hid its own controls while a request was pending**, so after the ten-minute window there was still no button to press, and `OMNISSA_UPDATE_CHECK_ENABLED=false` hid a pending request and the host's verdict along with the detection. The pending notice now sits alongside the controls (amber, with the reason, once stale); the verdict has a **Dismiss** for administrators, audited as `update-result-dismissed`; and turning off the daily check hides only the detection. An approval whose request could not be written leaves a compensating `update-approval-failed` audit row rather than a lone approval.
- **The updater's rollback could report a clean revert over an outage.** If the previous version did not come back up either, the verdict was still *rolled back*; it is now *rollback-failed*, and the Dashboard says so. The rollback target is the last version the host *proved* (`last-known-good`), not whatever the compose file says — a run killed between pinning and verifying no longer poisons the next one. The pin is rewritten by pattern at write time rather than by a line number captured earlier, and a health probe that cannot find the LAN address fails the run instead of silently trying loopback.
- **Nothing consumed an approval on a host that was not the ZimaCube.** The mount was there, the console approved, and the request was never read. `deploy/updater/install.sh` installs the same updater on any systemd host; the Dashboard now says *nothing has picked it up* after ten minutes either way.

### Changed
- **No more moving `major.minor` image tag.** `publish.yml` publishes `N.N.N` once from a `v*` tag, and `latest` plus a commit sha from `main`; nothing else. The only reason to publish `1.22` was so that someone could pin to it, and a moving pin is what the updater exists to end.
- The seven `OMNISSA_UPDATE_*` settings are listed in both `.env.example` files; Troubleshooting gains *A deployment was refused, failed, or rolled back*.
