...
Update by re-running `deploy.sh`, or opt in to Watchtower-based auto-updates via the `autoupdate` compose profile (disabled by default; daily, label-scoped) — see [docs/deployment.md](docs/deployment.md#automatic-updates-optional-disabled-by-default). The CasaOS **"Check and then update"** button works only when the container is pinned to a **version** tag — CasaOS treats `:latest` as always current and skips the pull. Set `OMNISSA_IMAGE_TAG` (e.g. `1.9`) in the compose directory's `.env`; see [CasaOS updates](docs/deployment.md#casaos-check-and-then-update).
